...
const Admin = () => {
  ...
  // Rafraîchir les messages quand on change d'onglet vers "messages"
  ...
  const fetchContactMessages = async () => {
    try {
      ...
      const messages = await api.get<ContactMessage[]>('/contact-messages/');
  ...
  const handleMarkAsRead = async (messageId: string) => {
    try {
      await api.patch(`/contact-messages/${messageId}/`, { is_read: true });
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 container py-8">
        <div className="animate-fade-in space-y-8">
          ...
          {/* Tabs */}
          <Tabs defaultValue="pending" className="space-y-6" onValueChange={handleTabChange}>
            <TabsList>
                            ...
                            <TableCell className="text-right">
                              <div className="flex justify-end gap-2">
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() => {
                                    // Afficher les détails du message dans une modal ou un dialogue
                                    const fullMessage = `Organisation: ${message.organization || 'Non renseignée'}\n\nMessage:\n${message.message}`;
                                    alert(fullMessage);
                                  }}
                                >
